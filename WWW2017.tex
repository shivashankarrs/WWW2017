...
Hyperparameters for the SVM ($C$) and biased-SVM ($C$ and class weights) were tuned using cross-validation. For our proposed method (``DP-SVM''), as described in \secref[s]{sec:querying} and \ref{sec:labelling}, we provide average scores across five runs with five random positive sets $S_{p}$. We present the micro-averaged F-score for the test set of the two datasets in \tabref{tab:results}. 
